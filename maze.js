...
function setup() {
    ...
    document.querySelector('canvas').style.top = "260px";
...
function draw() {
    ...
    if(playedMove && (!gameOver)) {
        ...
        document.querySelector(".timeClass").innerText = 'Time left: ' + (30 - timeCnt);
        document.querySelector(".MoveCnt").innerHTML = 'Moves: ' + moveCnt;

        if(current.i == cols-1 && current.j == rows-1 && (currentTime.getSeconds() <= (date.getSeconds() + 30))) {
            // console.log("Congratulations! You won");
            gameOver = true;
            win = true;
            document.querySelector(".timeClass").innerHTML = "Game Over"; 
        } else if(timeCnt >= 30) {
            // console.log('over');
            gameOver = true;
            document.querySelector(".timeClass").innerHTML = "Game Over";
    ...
    if(gameOver) {
        ...
        if(win) {
            fill(91, 68, 155);
            text('You Won!!', 150, 100, 300, 100);
        } else {
            fill(241, 14, 14);
            text('Oops! You Lose', 90, 100, 400, 100);
        }
        text('Game Over', 130, 300, 350, 100);
...
function keyPressed() {
    if(started && gridBuilt && (!gameOver)) {
        ...
        if(!playedMove) {
            playedMove = true;
            let timeIndicator = document.createElement('h1');
            timeIndicator.className = 'timeClass heading3';
            document.querySelector('.center').appendChild(timeIndicator);
        }
